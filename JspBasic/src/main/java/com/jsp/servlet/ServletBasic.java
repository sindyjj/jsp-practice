...
//			//요청 파라미터
//			String querString = request.getQueryString();// 아이디 비번 이름이 다 이어붙쳐져서 주소에 나온다.
//			//요청을 보낸 IP주소
...
//			System.out.println("queryString: " + querString);
//			System.out.println("addr: " + addr); 
//			*/
//			
//			//요청(request) 과 함께 전달된 파라미터를 낱개로 얻는 방법
//			String account =request.getParameter("account"); // 아이디를 따로 줘라!
//			String password =request.getParameter("password"); // 비밀번호를 따로 줘라!
//			String name =request.getParameter("name"); // 이름을 따로 줘라!
//			
...
//			response.setContentType("text/html");  // 응답할 파일을 세팅해줘야한다
//			response.setCharacterEncoding("UTF-8"); // 이걸 해줘야 한글이 깨지지 않는다.
//			
//			//응답 화면 제작
//			//자바 클래스에서 브라우저로 바로 응답을 구현하기 위해서는
//			//printWriter 객체를 사용합니다.(기본 제공을 받아서 사용하는 것)
//			// 응답은 HttpServletResponse response 얘 다.
//	
...
//	}
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		//메개값으로 내장객체 request와 response가 전달되므로
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		비밀번호: jj1023jj1023
		이름: ìì ì±           
		//decording  : 한글이 아닌 post으로 할 땐 인코딩을 1번 더 거쳐야한다.
		//  HttpServletRequest req 한테 req.setCharacterEncoding("UTF-8"); 요청해야한다. (EX 144번)
		 */
